...
router.post("/", async (req, res) => {
  try {
    ...
    // ✅ Log history (only create/update/delete/reply events)
    await addHistory(
      "CREATE_CONTACT",
...
// Get all contacts (admin only) 👉 ❌ No history logging here
...
router.put("/:id/read", auth, async (req, res) => {
  try {
    ...
    if (contact) {
      await addHistory(
        "UPDATE_CONTACT",
        `✏️ Contact marked as read (${contact.email}) by ${req.user.email}`,
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    if (deleted) {
      await addHistory(
        "DELETE_CONTACT",
        `🗑️ Contact deleted (${deleted.email}) by ${req.user.email}`,
...
router.post("/:id/reply", auth, async (req, res) => {
  try {
    ...
    await addHistory(
      "UPDATE_CONTACT",
      `📨 Reply sent to ${contact.email} by ${req.user.email}`,
      { contactId: contact._id, reply: message },
      req.user._id,
      req.ip
    );
