...
// Submit contact form
router.post("/", 
  historyMiddleware("CONTACT_SUBMISSION", (req, data) => `New contact message from: ${req.body.name} (${req.body.email})`),
  async (req, res) => {
    try {
      const { name, email, phone, message } = req.body;
...
router.get("/", 
  auth, 
  historyMiddleware("VIEW_CONTACTS", "Admin viewed contact messages list"),
  async (req, res) => {
    try {
      const contacts = await Contact.find().sort({ createdAt: -1 });
      res.json(contacts);
...
router.put("/:id/read", 
  auth, 
  historyMiddleware("MARK_CONTACT_READ", (req, data) => `Contact message marked as read from: ${data.name} (${data.email})`),
  async (req, res) => {
    try {
      const contact = await Contact.findByIdAndUpdate(
        req.params.id,
...
router.delete("/:id", 
  auth, 
  historyMiddleware("DELETE_CONTACT", (req, data) => {
    // We need to get contact info before deletion
    return new Promise(async (resolve) => {
      try {
        const contact = await Contact.findById(req.params.id);
        resolve(`Contact message deleted from: ${contact?.name || 'Unknown'} (${contact?.email || 'Unknown'})`);
      } catch (err) {
        resolve(`Contact message deleted (ID: ${req.params.id})`);
      }
    });
  }),
  async (req, res) => {
...
router.post("/:id/reply", 
  auth, 
  historyMiddleware("CONTACT_REPLY", (req, data) => `Admin replied to contact message from: ${data.contact?.name || 'Unknown'} (${data.contact?.email || 'Unknown'})`),
  async (req, res) => {
    try {
      const { message } = req.body;
      const contact = await Contact.findById(req.params.id);
      ...
      res.json({ 
        message: "Reply sent successfully (check console for preview link)",
        contact: contact // This will be used by history middleware
      });
    } catch (error) {
      console.error("Error sending reply:", error);
      res.status(500).json({ message: "Error sending reply" });
